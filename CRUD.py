...
def delete_product_from_products(product_id, product_name, product_price, product_filename, enabled):#Hecho por mi para eliminar productos
    connection = db.get_db()
    cur=connection.cursor()
    cur.execute("DELETE FROM products WHERE product_id= ? AND product_id= ?" , [product_id, product_name, product_price, product_filename, enabled])
    connection.commit()
    db.close_db()
