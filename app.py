...
def add_new_user_mail_sender():
    try:
        ...
        return render_template("admin-panel-users-add.html",Alert="Usuario creado correctamente.")
...
def modify_user(userId):
    ...
    if utils.isEmailValid(email) and utils.isUsernameValid(usuario) and (clave == "" or utils.isPasswordValid(clave)):
        ...
        return redirect("/admin/users/edit", Alert="Usuario modificado correctamente.")
...
def modify_product(productId):
    ...
    try:
        ...
        return redirect("/admin/products/edit", Alert="Producto Modificado con éxito")
        
    except:
        return render_template("admin-panel-products-error.html",Alert="Error en la actualización del producto. Favor verificar campos ingresados.")
